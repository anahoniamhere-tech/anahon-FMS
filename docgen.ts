...
/**
 * A position for an ARABIC line. A role title the system already translates (a seat in roles.ts, or
 * the Executive Director's title) prints in Arabic; free text keeps its English, isolated LTR so it
 * cannot reorder the Arabic around it. Never a guessed translation.
 */
const positionAr = (position: string) => {
  const p = String(position || "").trim();
  const known = (ALL_ROLES as string[]).includes(p) || p === "Executive Director";
  return known && AR[p] ? esc(AR[p]) : ltr(esc(p));
};
...
${noFixedValue
  ...
  /**
   * Language. Saad's decision, 12 Sep 2026: the Arabic text governs. Stated as its own article
   * rather than a footnote, because which text prevails is a term of the contract and not a
   * remark about it — and the Arabic is printed first for the same reason.
   */
...
<br><strong>This statement is bilingual, and the Arabic text governs.</strong> Every figure appears once only,
