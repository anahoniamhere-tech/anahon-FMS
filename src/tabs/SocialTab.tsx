...
export default function SocialTab({ state, currentUser, triggerToast, refreshState }: SharedProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Direct Meta client (3 Sep 2026) — shown only until Postiz is configured; retired in one commit once
          Postiz posts reliably to Facebook and Instagram (PLAN.md §4). */}
      {pz?.configured ? (
        <p className="text-xs text-slate-500">Direct posting from this desk is retired: publishing goes through Postiz now, released by the editorial gate above.</p>
      ) : (<>
        {/* connection */}
