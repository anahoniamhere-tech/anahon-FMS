...
ok("approval moves the money in one transaction", /Approval\. Everything re-checked[\s\S]{0,2500}prisma\.\$transaction/.test(server));
...
ok("a top-up draws only on an active bank account", /source\.type !== "Bank" \|\| !source\.active/.test(server));
...
ok("on or after it, other cash is undecided — null, not a quiet 1120", cashLedgerFor("2026-10-01", "2026-09-20", false) === null);
