...
{
  const fx = read("src/fxPairs.ts");
  ok("FX clearing is swept only for conversions whose two legs are both on statements; reversals always sweep",
    /pairedIds = new Set<string>\(legs\.filter\(l => l\.reversal\)\.map\(l => l\.id\)\)/.test(fx) && /legs\.filter\(l => l\.eur && !l\.reversal\)/.test(fx)
    && /!u\.eur && !u\.reversal && !pairedIds\.has\(u\.id\) && u\.type !== e\.type/.test(fx)
    && /const \{ pairedIds, unpaired \} = pairFxLegs\(fxLegs\)/.test(rebuild) && /let fxNet = r2\(fxLegs\.filter\(l => pairedIds\.has\(l\.id\)\)/.test(rebuild));
}
